...
$(function () {
  ...
  sendHttpRequest(
    "GET",
    "https://ytuce-sab.herokuapp.com/api/survey/" + surveyParam
  ).then((data) => {
    $survey.append(`
    ...
    $.each(data.questions, function (i, question) {
      if (question.qType === 1) {
        ...
        $.each(question.qAnswer, function (t, answer) {
          $("." + question._id).append(
            `
                    <input class="radio-option only-choice" type="radio" name="${
                      question._id
                    }" value="${t + 1}"/>${answer.text}<br>
                        `
          );
        });
        $surveyBox.append(`
        ...
        $.each(question.qAnswer, function (t, answer) {
          $("." + question._id).append(
            `
                        <input class="multi-choice" type="checkbox" name="${
                          question._id
                        }" id="${answer._id}" value="${t + 1}" />
                        <label class="checkbox-option" for="${answer._id}"> ${
              answer.text
            }</label><br />
                        `
          );
...
$("form.survey").on("submit", function () {
  ...
  sendHttpRequest(
    "POST",
    "https://ytuce-sab.herokuapp.com/api/answer/add",
    data
  ).then((response) => {
