const sendHttpRequest = (method, url, data) => {
  const promise = new Promise((resolve, reject) => {
    ...
    xhr.setRequestHeader(
      "token",
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI1ZWFiNDdjMzExOWFkZDcwOGRmOTdmM2QiLCJpYXQiOjE1ODgyODMzOTJ9.6kWW9TbJdYCaPY5zKRPfVynPBxpVF-39V7tLMGUdKGg"
    );
...
function addFunction() {
  ...
  if (
    document.getElementById("select-box").value == 2 ||
    document.getElementById("select-box").value == 3
  ) {
    cln
      .getElementsByClassName("btnOption")[0]
      .addEventListener("click", addOptionFunction);
  }
  document.getElementById("myList1").appendChild(cln);

  cln
    .getElementsByClassName("btnSaveQuestion")[0]
    .addEventListener("click", saveQuestion);
}

addOptionFunction = function () {
  var item = document
    .getElementById("firstQuestion")
...
function saveSurvey() {
  ...
  try {
    ...
    finishDate = new Date(finishDate.split("-").reverse().join("-"))
      .getTime()
      .toString();
  ...
  sendHttpRequest(
    "POST",
    "https://ytuce-sab.herokuapp.com/api/survey/add",
    data
  ).then((response) => {
